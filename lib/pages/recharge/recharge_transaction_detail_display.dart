...
class RechargeTransactionDetailDisplay extends StatefulWidget {
  final TopUpTransaction trans;
  final RechargeTransactionDetails tranDetails;

  const RechargeTransactionDetailDisplay({
    super.key, required this.trans,
    required this.tranDetails,
  });

  @override
  RechargeTransactionDetailDisplayState createState() => RechargeTransactionDetailDisplayState();
}

class RechargeTransactionDetailDisplayState extends State<RechargeTransactionDetailDisplay> {
  ...
  Future<void> setDocumentPath() async {
    ...
    if(mounted) {
      int date = (DateTime.parse(widget.trans.transactionDate!)).millisecondsSinceEpoch;
      setState(() {
